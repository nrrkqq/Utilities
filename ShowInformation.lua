...
return function(Library, Framework, GetFlag)
	local RunService = game:GetService("RunService")
	local Players = game:GetService("Players")
	local LocalPlayer = Players.LocalPlayer

	local Green = Color3.fromRGB(76, 255, 76)
	...
	task.spawn(function()
		while RunService.RenderStepped:Wait() do
			if ShowingInfo then
				local ShowingCharacter = ShowingInfo and Framework:GetCharacter(ShowingInfo)
				local OurCharacter = Framework:GetCharacter(LocalPlayer)

				if ShowingCharacter and OurCharacter then
					local ShowingRoot = Framework:GetRoot(ShowingCharacter)
					local OurRoot = Framework:GetRoot(OurCharacter)

					if ShowingRoot and OurRoot then
						local Camera = workspace.CurrentCamera
						local screenPos, onScreen = Camera:WorldToViewportPoint(ShowingRoot.Position)

						if onScreen then
							Outline.Position = UDim2.new(0, screenPos.X, 0, screenPos.Y + 220)
	...
	function InformationWindow:Show(Player)
		...
		self:SetVisible(true) -- cannot access 'instance' (lacking capability of plugin)
